...
fn apply_env_overrides(raw: &mut RawConfig) {
    ...
    override_from_env!(raw.local.cert_fingerprint, "PUBLISHER_LOCAL_CERT_FINGERPRINT");
...
pub fn normalise_fingerprint(s: &str) -> Result<String, String> {
    ...
    if clean.len() == 64 && clean.chars().all(|c| c.is_ascii_hexdigit()) {
        ...
        Err("local.cert_fingerprint must be a SHA-256 fingerprint \
             (64 hex characters, colons and spaces allowed)".to_owned())
    }
}
...
fn build_and_validate(raw: RawConfig) -> Result<PublisherConfig, ConfigError> {
    ...
    let destination_raw = raw.publisher.destination
        .filter(|s| !s.is_empty());
    let destination = match destination_raw.as_deref() {
        None => Destination::RaceControl,
        Some(s) => Destination::parse(s).ok_or_else(|| ConfigError::Validation(format!(
    ...
    let local = if destination == Destination::Local {
        let require_local = |field: Option<String>, key: &str, env_var: &str| -> Result<String, ConfigError> {
            match field.filter(|s| !s.is_empty()) {
                Some(v) => Ok(v),
                None => Err(ConfigError::Validation(format!(
        ...
        let cert_fingerprint = raw.local.cert_fingerprint
            .filter(|s| !s.is_empty())
            .map(|fp| normalise_fingerprint(&fp).map_err(ConfigError::Validation))
            .transpose()?;

        Some(LocalConfig { url, token, cert_fingerprint })
    } else {
        None
    };
...
client_secret = "s"
"#);
        assert_eq!(cfg.auth.as_ref().unwrap().scope, "api://racecontrol-api-a780e279-1cb6-4ed0-9ef6-49029aa50a42/.default");
...
"#);
        ...
        assert!(cfg.auth.is_none(), "local destination must not require auth");
...
"#).expect("valid toml");
    ...
    fn destination_parse_is_case_insensitive() {
        ...
        assert_eq!(Destination::parse("RACECONTROL"), Some(Destination::RaceControl));
    ...
    fn fingerprint_normalisation() {
        ...
        // whitespace stripped
        assert_eq!(
            normalise_fingerprint(&format!("  {hex} \n")).unwrap(),
            hex
        );
    ...
    fn env_overrides_win_over_file_for_local() {
        ...
        let raw: RawConfig = toml::from_str(&format!(r#"
[publisher]
